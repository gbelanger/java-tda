...
public final class Utils {
    ...
    private static void fillGapsRecursively(DoubleArrayList dataList) {
		...
		if (indexListOfNaNs.size() > 0) {
			...
			double[] avgAndVar = Stats.getAvgAndVarOfDiffsBetweenBins(dataList);
			...
			while (i < indexListOfNaNs.size()) {
				...
				if (gapSize == 1) {
					...
					avgAndVar = Stats.getAvgAndVarOfDiffsBetweenBins(bothDataBlocks);
	...
	// 	return Stats.getMean(getSpacings(data)); 
    // }
	...
	// 	double mean = Stats.getMean(data);
	// 	for (int i=0; i < data.length; i++)
